...
type Props = {
  ...
  onFinalized: (voice: Voice) => void;
};

export function VoiceInterviewPanel({ voiceId, initialMessages, onFinalized }: Props) {
  ...
  async function sendMessage(text: string) {
    ...
    try {
      const response = await fetch(`/api/voices/${voiceId}/interview`, {
        method: "POST",
  ...
  async function handleFinalize() {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/voices/${voiceId}/interview/finalize`, {
        method: "POST",
      });
      ...
      onFinalized(body.voice as Voice);
    } catch (err) {
  ...
  return (
    <Card className="flex flex-col gap-3">
      <h2 className="font-medium">Voice Interview</h2>
      <p className="text-xs text-muted">
        A quick back-and-forth about tone, vocabulary, pacing, humor, and quirks — the AI turns
        this into the voice&apos;s Brain, same as Generate with AI but conversational.
      </p>
      ...
      >
        {isFinalizing ? "Finalizing…" : "Finalize voice from interview"}
      </Button>
    </Card>
  );
}
